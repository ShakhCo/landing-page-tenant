  ...
  return (
    <div
      className={`relative flex h-full flex-col rounded-3xl p-7 md:p-8 ${
        plan.highlighted
          ? "bg-white shadow-xl shadow-[var(--accent)]/15 ring-2 ring-[var(--accent)] lg:-mt-4 lg:pb-12"
          : "bg-gray-50"
      }`}
    >
      {plan.badge && (
          ...
          <span className="relative inline-block">
            <AnimatePresence mode="popLayout">
              <motion.span
                ...
                className="block text-4xl font-extrabold tracking-tight text-gray-900"
              >
                {formatUZS(monthly)}
              </motion.span>
            </AnimatePresence>
          </span>
          <span className="text-sm font-medium text-gray-500">UZS / oy</span>
        </motion.div>
            ...
            <li key={feature.text} className="flex items-start gap-2.5">
              <Check
                className={`mt-0.5 size-4 shrink-0 ${
                  feature.highlight ? "text-[var(--accent)]" : "text-gray-400"
                }`}
                strokeWidth={2.5}
              />
              <span
