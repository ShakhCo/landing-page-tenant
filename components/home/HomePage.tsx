...
export function HomePage({ dict, locale }: { dict: HomeDict; locale: Locale }) {
  return (
    <div className="min-h-screen overflow-x-clip bg-white text-[oklch(0.18_0_0)]">
      ...
      <main className="pb-8 md:pb-24">
        <div className="mx-auto max-w-[1360px] px-5">
          <HeroGrid d={dict.hero} />
          <SolutionsSection d={dict.solutions} />
          <AcquiringSection sticky={dict.sticky} />
          <SupportCtaSection d={dict.supportCta} />
        </div>
        <IntegrationsSection d={dict.integrations} />
        <div className="mx-auto max-w-[1360px] px-5">
          <ProSection d={dict.pro} />
          <FaqSection d={dict.faq} />
        </div>
      </main>
...
function HeroGrid({ d }: { d: HomeDict["hero"] }) {
  return (
    <section className="grid gap-4 md:grid-cols-3 md:grid-rows-2 md:gap-5">
      ...
      <article className="group relative grid items-center gap-6 overflow-hidden rounded-3xl bg-gray-100 p-6 transition-shadow duration-300 hover:shadow-sm sm:p-8 md:col-span-2 md:grid-cols-[1fr_minmax(0,46%)] md:p-10">
        <div className="flex flex-col justify-center">
          <h1 className="text-2xl font-extrabold leading-tight tracking-tight md:text-3xl">
            {d.card1.titleLine1}<br /><span className="text-gray-500">{d.card1.titleLine2Accent}</span>
          </h1>
      ...
      <article className="group relative flex flex-col overflow-hidden rounded-3xl bg-gray-100 py-8 px-6 transition-shadow duration-300 hover:shadow-sm md:col-start-3 md:row-span-2">
        <div className="shrink-0 px-2">
          <h3 className="text-2xl font-extrabold leading-tight tracking-tight md:text-3xl">
            <span className="text-gray-500">{d.card2.titleLine1Accent}</span><br />{d.card2.titleLine2}
          </h3>
      ...
      <article className="relative flex flex-col overflow-hidden rounded-3xl bg-gray-100 p-6 transition-shadow duration-300 hover:shadow-sm sm:p-8">
        <div className="flex items-start justify-between">
          <h3 className="text-2xl font-extrabold leading-tight tracking-tight md:text-3xl">
            <span className="text-gray-500">{d.card3.titleLine1Accent}</span><br />{d.card3.titleLine2}
          </h3>
      ...
      <article className="relative flex flex-col overflow-hidden rounded-3xl bg-gray-100 p-8 transition-shadow duration-300 hover:shadow-sm">
        <div className="flex items-start justify-between">
          <h3 className="text-2xl font-extrabold leading-tight tracking-tight md:text-3xl">
            {d.card4.titleLine1}<br /><span className="text-gray-500">{d.card4.titleLine2Accent}</span>
          </h3>
          <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white shadow-sm">
            <IconUserCard />
          </span>
...
function AcquiringSection({ sticky }: { sticky: HomeDict["sticky"] }) {
  ...
  return (
    <section className="mt-24">
      ...
      <StickyFeatureSection dict={sticky} />
    </div>
...
function SupportCtaSection({ d }: { d: HomeDict["supportCta"] }) {
  const cta = (
    <svg
      width="14"
      height="14"
  ...
  const renderRow = (prefix: string) =>
    Array.from({ length: 5 }).flatMap((_, i) => [
      ...
      >
        <span className="text-gray-400">{d.marqueeAccent}</span>{" "}
        <span className="text-gray-900">{d.marqueeRest}</span>
      </span>,
      <span
        key={`${prefix}-dot-${i}`}
      ...
      <div className="mt-16 grid gap-5 md:grid-cols-2">
        <article className="flex flex-col rounded-3xl bg-gray-50 p-8 md:p-10">
          <h3 className="text-2xl font-bold tracking-tight md:text-3xl">
            {d.card1.titlePre}{" "}
            <span className="text-[var(--accent)]">{d.card1.titleAccent}</span>{" "}
            {d.card1.titlePost}
          </h3>
          <p className="mt-4 text-base text-gray-600">
            {d.card1.p1Pre}{" "}
            ...
            {d.card1.p1Mid}{" "}
            <span className="font-semibold text-red-600">{d.card1.p1Risk}</span>
            {d.card1.p1Post}
          </p>
          <p className="mt-3 text-base text-gray-600">
            {d.card1.p2Pre}{" "}
            <span className="font-bold text-gray-900">{d.card1.p2Free}</span>.
          </p>
          <a
            href="#"
        ...
        <article className="flex flex-col rounded-3xl bg-gray-50 p-8 md:p-10">
          <h3 className="text-2xl font-bold tracking-tight md:text-3xl">
            {d.card2.titlePre}{" "}
            <span className="text-[var(--accent)]">{d.card2.titleAccent}</span>{" "}
            {d.card2.titlePost}
          </h3>
          <p className="mt-4 text-base text-gray-600">
            {d.card2.p1Pre}{" "}
...
/** Visual config per card — copy comes from the dictionary, zipped by index. */
...
function IntegrationsSection({ d }: { d: HomeDict["integrations"] }) {
  const [carouselApi, setCarouselApi] = useState<CarouselApi>();
  const [canScrollPrev, setCanScrollPrev] = useState(false);
  const [canScrollNext, setCanScrollNext] = useState(false);
  const [currentSlide, setCurrentSlide] = useState(0);

  const integrations: Integration[] = INTEGRATION_STYLES.map((style, i) => ({
    ...style,
    name: d.items[i].name,
    title: d.items[i].title,
    description: d.items[i].description,
    bullets: [...d.items[i].bullets],
  }));
        ...
        <div className="mb-10 flex items-end justify-between gap-6 md:mb-14 lg:mb-16">
          <div className="flex max-w-3xl flex-col gap-4">
            ...
            <h2 className="text-3xl font-extrabold leading-tight tracking-tight text-gray-900 md:text-4xl lg:text-5xl">
              {d.titleMain}{" "}
              <span className="text-gray-500">{d.titleAccent}</span>
            </h2>
        ...
        <div className="mt-8 flex justify-center gap-2">
          {integrations.map((it, index) => (
            <button
              ...
              aria-label={`${d.slideLabel} ${index + 1}: ${it.name}`}
            />
          ))}
        </div>
      </div>
...
function ProSection({ d }: { d: HomeDict["pro"] }) {
      ...
      <div className="relative grid items-center gap-10 lg:grid-cols-[1fr_minmax(0,42%)] lg:gap-16">
        <div>
          ...
          <h2 className="mt-5 text-3xl font-bold leading-[1.1] tracking-tight text-white md:text-4xl lg:text-5xl">
            {d.titleLine1}
            <br />
            <span className="text-[var(--accent)]">{d.titleAccent}</span>{" "}
            {d.titlePost}
          </h2>
          ...
          <div className="mt-9 flex flex-wrap gap-2 md:gap-3">
            ...
            >
              ...
              <div className="text-left leading-tight">
                ...
                <p className="text-sm font-semibold md:text-base">{d.ios}</p>
              </div>
            </a>
            <a
              href="#"
            ...
            >
              ...
              <div className="text-left leading-tight">
                ...
                <p className="text-sm font-semibold md:text-base">{d.android}</p>
              </div>
            </a>
          </div>
        </div>
...
function FaqSection({ d }: { d: HomeDict["faq"] }) {
